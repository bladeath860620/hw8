...
void memory_to_ip(ip *memory, unsigned int adr[], FILE *timing)
{
	if(memory != NULL)
	{
...
void print_list(ip *head, FILE *timing, unsigned int adr[] ,unsigned int length)
{
	ip *temp = head;
	while(temp != NULL)
	{
		memory_to_ip(temp, adr, timing);
		fprintf(timing, "%u.%u.%u.%u/%u\n", adr[0], adr[1], adr[2], adr[3], temp -> length);
		temp = temp -> next;
	}
	return;
}
...
int main()
{
	...
	FILE *k400, *result, *search, *insert, *delete, *timing;
	...
	/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^create^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
	for(i = 0; i < 1 << 8;i++) {
		print_list(tab1[i], timing, adr, length);
		fprintf(timing, "==============================\n");
	}
	for(i = 0;i < 1 << 12;i++) {
		print_list(tab2[i], timing, adr, length);
		fprintf(timing, "==============================\n");
		print_list(tab3[i], timing, adr, length);
	}
	/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^test^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
	ip *memory;
	while(fgets(string, 20, search) != NULL)
	{
		...
		{
			memory = searching(tab3[int_adr], bin(adr));
			memory_to_ip(memory ,adr ,timing);
			length = memory -> length;
